...
function monthDiff_Local(d_new, d_old) {
    let difdt = new Date(d_new - d_old);
    let years = parseInt(difdt.toISOString().slice(0, 4) - 1970);
    let months = parseInt(difdt.getMonth()) + years * 12;
    return (months.toString() + "M&nbsp;" + difdt.getDate() + "D");
}

function timeDiff_Local(d_new, d_old) {
    let difMs = d_new - d_old;
    let diffHrs = Math.floor(difMs / 3600000); // hours
    let diffMins = Math.round((difMs % 3600000) / 60000); // minutes
...
module.exports = {
    getMachineTypeName: function(type) {
        switch (type) {
            case 'gui':
                return 'Gear Side > Upper > In ';
            case 'guo':
                return 'Gear Side > Upper > Out ';
            case 'gdi':
                return 'Gear Side > Down > In ';
            case 'gdo':
                return 'Gear Side > Down > Out ';
            case 'dui':
                return 'Dab Side > Upper > In ';
            case 'duo':
                return 'Dab Side > Upper > Out ';
            case 'ddi':
                return 'Dab Side > Down > In ';
            case 'ddo':
                return 'Dab Side > Down > Out ';
            default:
                return 'undefined';
        }
    },
    ...
    machineBearingMakeTable: function(bearing, startingDate = new Date("2000-01-02"), endingDate = new Date("9999-01-02")) {
        ...
        for (i = 0; i < len; ++i) {
            ...
            switch (bearing[i].type) {
                ...
                case "gui":
                    ...
                    break;
                default:
                    return `<div class="alert alert-danger alert-dismissable fade in"><a aria-label=close class=close data-dismiss=alert href=#>×</a><strong>ERROR! </strong>Invalid Bearing Type : ${bearing[i].type}</div>`;
            }
        ...
        if (unique_date_len === 0)
            return `<div class="alert alert-dismissable alert-warning fade in"><a aria-label=close class=close data-dismiss=alert href=#>×</a> <strong>No Entry Found In The Given Time Frame!</strong></div>`;
        else
            return htmlToSend;
    },
    updateMachineBearingMakeTable: function(bearing, startingDate = new Date("2000-01-02"), endingDate = new Date("9999-01-02"), machineNameCheck = "", purpose = "normal") {
        ...
        // filling the arrays for the letious types
        let DDO_ans = makeTableBearing(unique_date, unique_date_len, DDO, true),
    ...
    machineBackUpRollMakeTable: function(backUpRoll, starting_date = new Date("2000-01-02"), ending_date = new Date("9999-01-02"), machineNameCheck = "", purpose = "normal") {
        ...
        let len = backUpRoll.length,
            i = 0,
            htmlToSend;
        if (purpose === "normal")
            htmlToSend = `<div class="table container-fluid"><table class="table table-bordered table-condensed table-striped"><tr><th>S No.<th>Date<th colspan=2>Back Up Roll Up<th class=emptyInMiddle><th colspan=2>Back Up Roll Down<tr style="border-bottom: thick solid black;"><th><th><th>Diameter<th>Comment<th class=emptyInMiddle><th>Diameter<th>Comment</tr>`;
        else {
            htmlToSend = `<div class="table container-fluid"><table class="table table-bordered table-condensed table-striped"><tr><th>S No.<th>Date<th colspan=3>Back Up Roll Up<th class=emptyInMiddle><th colspan=3>Back Up Roll Down<tr style="border-bottom: thick solid black;"><th><th><th>Diameter<th>Comment<th><th class=emptyInMiddle><th>Diameter<th>Comment<th></tr>`;
            for (i = 0; i < len; ++i) {
        ...
        for (i = 0; i < len; ++i) {
            ...
            if (backUpRoll[i].type === "1") {
                ...
                htmlToSend += `<td>${backUpRoll[i].dia}</td><td>${backUpRoll[i].comment}</td>`;
                if (purpose !== "normal") {
                    htmlToSend += `<td> <a name="${(purpose==="updateexisting")?"updateEntryAnchorClickSelector":"deleteEntryAnchorClickSelector"}" href="/updateEntry/${(purpose==="updateexisting")?"updateBackUpRollEntry":"deleteBackUpRollEntry"}?index=${encodeURIComponent(backUpRoll[i].index)}&type=2&name=${encodeURIComponent(machineNameCheck)}&date=${encodeURIComponent(backUpRoll[i].date)}&dia=${encodeURIComponent(backUpRoll[i].dia)}&comment=${encodeURIComponent(backUpRoll[i].comment)}" class="btn btn-white btn-info btn-xs"> <span class="glyphicon glyphicon-${(purpose==="updateexisting")?"pencil":"remove"}"></span> ${(purpose==="updateexisting")?"Update":"Delete"} </a></td>`;
                }
            }
            htmlToSend += `</tr>`;
        }
    ...
    machineJobWorkMakeTable: function(jobWork, starting_date = new Date("2000-01-02"), ending_date = new Date("9999-01-02"), machineNameCheck = "") {

        let len = jobWork.length,
            i = 0,
            htmlToSend = `<div class="table container-fluid"><table class="table table-bordered table-condensed table-striped"><tr><th>S No.<th>Date<th>By Whom<th>Comment</tr>`;

        if (len === 0)
            return `<br><h2 style="text-align:center;color:red;">No Job Work Transaction Exists For This Machine !!</h2><br>`;

        jobWork.sort(function(a, b) {
            let diff = a.date.getTime() - b.date.getTime();
            if (diff != 0)
                return diff;
            else
        ...
        let counter = 1;
        for (i = 0; i < len; ++i) {
            let curr_date = jobWork[i].date;
            if (curr_date < starting_date || curr_date > ending_date)
                continue;

            htmlToSend += `<tr><td>${counter++}</td><td>${formattedDate_Local(jobWork[i].date)}</td><td>${jobWork[i].whom}</td><td>${jobWork[i].comment}</td></tr>`;
        }
        htmlToSend += `</table></div>`;
        return htmlToSend;
    },
    machineWorkRollMakeTable: function(workRoll, status, startingDate = new Date("2000-01-02"), endingDate = new Date("9999-01-02"), rollNameCheck = "", purpose = "normal") {
        let len = workRoll.length,
            i = 0,
            htmlToSend;
        if (purpose === "normal")
            htmlToSend = `<pre class="container" style="text-align:center;"><h5>WORK ROLL STATUS :- ${status}</h5></pre><div class="table container-fluid"><table class="table table-bordered table-condensed table-striped"><tr><th>S No.<th>Mach. No.<th colspan=2>Mach. Up<th colspan=2>Mach. Down<th>Working Time<th style="border-right: thick solid red;">Reason<th colspan=2>Grinding Ending Time<th>Cooling Time<th>Diameter<th>Grinding Operator<th>Comment<tr style="border-bottom: thick solid black;"><th><th><th>Date<th>Time<th>Date<th>Time<th><th style="border-right: thick solid red;"><th>Date<th>Time<th><th><th><th></tr>`;
        else {
            htmlToSend = `<pre class="container" style="text-align:center;"><h5>WORK ROLL STATUS :- ${status}</h5></pre><div class="table container-fluid"><table class="table table-bordered table-condensed table-striped"><tr><th>S No.<th><th>Mach. No.<th colspan=2>Mach. Up<th colspan=2>Mach. Down<th>Working Time<th style="border-right: thick solid red;">Reason<th colspan=2>Grinding Ending Time<th>Cooling Time<th>Diameter<th>Grinding Operator<th>Comment<tr style="border-bottom: thick solid black;"><th><th><th><th>Date<th>Time<th>Date<th>Time<th><th style="border-right: thick solid red;"><th>Date<th>Time<th><th><th><th></tr>`;
            for (i = 0; i < len; ++i) {
                workRoll[i].index = i;
            }
        }
...
${(purpose==="normal")?"":`<td><a name="${(purpose==="updateexisting")?"updateEntryAnchorClickSelector":"deleteEntryAnchorClickSelector"}" href="/updateEntry/${(purpose==="updateexisting")?"updateWorkRollEntry":"deleteWorkRollEntry"}?index=${encodeURIComponent(workRoll[i].index)}&name=${encodeURIComponent(rollNameCheck)}&m_no=${encodeURIComponent(workRoll[i].m_no)}&m_up=${encodeURIComponent(workRoll[i].m_up)}&m_down=${encodeURIComponent(workRoll[i].m_down)}&reason=${encodeURIComponent(workRoll[i].reason)}&g_time=${encodeURIComponent(workRoll[i].g_time)}&dia=${encodeURIComponent(workRoll[i].dia)}&operator=${encodeURIComponent(workRoll[i].operator)}&comment=${encodeURIComponent(workRoll[i].comment)}" class="btn btn-white btn-info btn-xs"> <span class="glyphicon glyphicon-${(purpose==="updateexisting")?"pencil":"remove"}"></span> ${(purpose==="updateexisting")?"Update":"Delete"} </a></td>`}
                    ...
                    prev_m_down_time = workRoll[i].m_down;
                }
                else {
                    htmlToSend += `<tr>
                           <td>${counter++}</td>
${(purpose==="normal")?"":`<td><a name="${(purpose==="updateexisting")?"updateEntryAnchorClickSelector":"deleteEntryAnchorClickSelector"}" href="/updateEntry/${(purpose==="updateexisting")?"updateWorkRollEntry":"deleteWorkRollEntry"}?index=${encodeURIComponent(workRoll[i].index)}&name=${encodeURIComponent(rollNameCheck)}&m_no=${encodeURIComponent(workRoll[i].m_no)}&m_up=${encodeURIComponent(workRoll[i].m_up)}&m_down=${encodeURIComponent(workRoll[i].m_down)}&reason=${encodeURIComponent(workRoll[i].reason)}&g_time=${encodeURIComponent(workRoll[i].g_time)}&dia=${encodeURIComponent(workRoll[i].dia)}&operator=${encodeURIComponent(workRoll[i].operator)}&comment=${encodeURIComponent(workRoll[i].comment)}" class="btn btn-white btn-info btn-xs"> <span class="glyphicon glyphicon-${(purpose==="updateexisting")?"pencil":"remove"}"></span> ${(purpose==="updateexisting")?"Update":"Delete"} </a></td>`}
                           ...
                           <td>${formattedDate_Local(workRoll[i].g_time)}</td>
                           <td>${formattedTime_Local(workRoll[i].g_time)}</td>
                           <td>${timeDiff_Local(workRoll[i].g_time,prev_m_down_time)}</td>
                           <td>${workRoll[i].dia}</td>
                           <td>${workRoll[i].operator}</td>
                           <td>${workRoll[i].comment}</td>
                      </tr>`;
