...
module.exports = {
  ...
  machineWorkRollMakeTable : function (workRoll , status, startingDate = new Date("2000-01-02"), endingDate = new Date("9999-01-02"), rollNameCheck = "" , purpose = "normal") {
    var len = workRoll.length,
        ...
        htmlToSend;
    if(purpose === "normal")
      htmlToSend = `<pre class="container" style="text-align:center;"><h5>WORK ROLL STATUS :- ${status}</h5></pre><div class="table container-fluid"><table class="table table-bordered table-condensed table-striped"><tr><th>S No.<th>Mach. No.<th colspan=2>Mach. Up<th colspan=2>Mach. Down<th>Working Time<th style="border-right: thick solid red;">Reason<th colspan=2>Grinding Starting Time<th colspan=2>Grinding Ending Time<th>Cooling Time<th>Diameter<th>Grinding Operator<th>Comment<tr style="border-bottom: thick solid black;"><th><th><th>Date<th>Time<th>Date<th>Time<th><th style="border-right: thick solid red;"><th>Date<th>Time<th>Date<th>Time<th><th><th><th></tr>`;
    else {
      htmlToSend = `<pre class="container" style="text-align:center;"><h5>WORK ROLL STATUS :- ${status}</h5></pre><div class="table container-fluid"><table class="table table-bordered table-condensed table-striped"><tr><th>S No.<th><th>Mach. No.<th colspan=2>Mach. Up<th colspan=2>Mach. Down<th>Working Time<th style="border-right: thick solid red;">Reason<th colspan=2>Grinding Starting Time<th colspan=2>Grinding Ending Time<th>Cooling Time<th>Diameter<th>Grinding Operator<th>Comment<tr style="border-bottom: thick solid black;"><th><th><th><th>Date<th>Time<th>Date<th>Time<th><th style="border-right: thick solid red;"><th>Date<th>Time<th>Date<th>Time<th><th><th><th></tr>`;
      for(i=0; i<len; ++i) {
        workRoll[i].index = i;
      }
    }
...
${(purpose==="normal")?"":`<td><a name="${(purpose==="updateexisting")?"updateEntryAnchorClickSelector":"deleteEntryAnchorClickSelector"}" href="/updateEntry/${(purpose==="updateexisting")?"updateWorkRollEntry":"deleteWorkRollEntry"}?index=${encodeURIComponent(workRoll[i].index)}&name=${encodeURIComponent(rollNameCheck)}&m_no=${encodeURIComponent(workRoll[i].m_no)}&m_up=${encodeURIComponent(workRoll[i].m_up)}&m_down=${encodeURIComponent(workRoll[i].m_down)}&reason=${encodeURIComponent(workRoll[i].reason)}&g_stime=${encodeURIComponent(workRoll[i].g_stime)}&g_time=${encodeURIComponent(workRoll[i].g_time)}&dia=${encodeURIComponent(workRoll[i].dia)}&operator=${encodeURIComponent(workRoll[i].operator)}&comment=${encodeURIComponent(workRoll[i].comment)}" class="btn btn-white btn-info btn-xs"> <span class="glyphicon glyphicon-${(purpose==="updateexisting")?"pencil":"remove"}"></span> ${(purpose==="updateexisting")?"Update":"Delete"} </a></td>`}
                           ...
                           <td>${formattedDate_Local(workRoll[i].g_stime)}</td>
                           <td>${formattedTime_Local(workRoll[i].g_stime)}</td>
                           <td>${formattedDate_Local(workRoll[i].g_time)}</td>
                           <td>${formattedTime_Local(workRoll[i].g_time)}</td>
                           <td>${timeDiff_Local(workRoll[i].g_stime,workRoll[i].m_down)}</td>
